...
const Footer = () => {
  ...
  const liveSupportModalOpen = useSelector(
    (state) => state.settings.liveSupportModalOpen
  );
  ...
  const liveSupportModal = () => {
    dispatch({ type: "settings/liveSupportModalOpen" });
  };
  ...
  const onClickConnect = () => {
    if (status) {
      if (user.role !== "Admin") joinChatRoom(user.role, "LiveSupport");
      if (user.role === "Admin") joinChatRoom(user.role, "LiveSupport");
    }
  };
    ...
    <>
      ...
      <footer id="footer_one">
        ...
        <div
          ...
          style={{ cursor: "pointer", right: "90px" }}
